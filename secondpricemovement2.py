...
def fetch_data_based_on_option():
    ...
    if earnings_data:
        ...
        for earnings_date, next_day, *rest in earnings_data:
            ...
            df = fetch_and_filter_data(ticker, multiplier, timespan, start_date, end_date, api_key, session)
            if df is not None:
...
def search_stock(event=None):
    ...
    for earnings_date, next_day, *rest in earnings_data:
        # Construct start and end dates for filtering
        start_date = earnings_date
        end_date = (datetime.fromtimestamp(next_day) + timedelta(days=1)).strftime('%Y-%m-%d')
        ...
        # Filter df for the earnings date and the day after
        filtered_df = df[(df.index.strftime('%Y-%m-%d') >= start_date) & (df.index.strftime('%Y-%m-%d') <= end_date)]
