...
class StockInfoApp:
    def __init__(self):
        ...
        self.ticker_frame = sg.Column([
            [sg.Text('Ticker:', text_color='white'),
             ...
             sg.Button('More', key='-MORE-', button_color=('white', 'blue'), pad=(5, 5))  # Added buffer between Search and More
             ]
    ...
    def show_stock_info(self, ticker):
        ...
        try:
            ...
            self.window['-f1-'].print(f"Error fetching data: {e}")
    ...
    def polygon_api(self, api_key, ticker, multiplier, timespan, start_date, end_date):
        ...
        if response.status_code == 200:
            ...
            print("Failed to fetch data from Polygon API")
    ...
    def get_earnings_prices(self, ticker, date):
        try:
            # Ensure the date format matches the expected format for yfinance
            formatted_date = datetime.strptime(date, '%Y-%m-%d').strftime('%Y-%m-%d')
            
            # Fetch historical prices for the earnings date and two days after
            start_date = datetime.strptime(date, '%Y-%m-%d') - timedelta(days=1)
            end_date = datetime.strptime(date, '%Y-%m-%d') + timedelta(days=2)
            earnings = yf.Ticker(ticker).history(start=start_date, end=end_date)
            
            if not earnings.empty:
                earnings_low = "${:.2f}".format(round(earnings['Low'].min(), 2))
                earnings_high = "${:.2f}".format(round(earnings['High'].max(), 2))
                return earnings_low, earnings_high
            else:
                return 'N/A', 'N/A'
        except Exception as e:
            print(f"Error fetching earnings low/high for {date}: {e}")
            return 'N/A', 'N/A'
    ...
    def show_earnings_info(self, ticker):
        ...
        if earnings_data:
            ...
            for _, low, high, eps, actual in earnings_data:
                if low != 'N/A' and low != '--':
                    earnings_low.append(float(low.replace('$', '')))
                else:
                    earnings_low.append(0.00)  # Replace 'N/A' or '--' with 0.00 or any other default value
                
                if high != 'N/A' and high != '--':
                    earnings_high.append(float(high.replace('$', '')))
                else:
                    earnings_high.append(0.00)  # Replace 'N/A' or '--' with 0.00 or any other default value
                
                if eps != 'N/A' and eps != '--':
                    estimated_eps.append(float(eps.replace('$', '')))
                else:
                    estimated_eps.append(0.00)  # Replace 'N/A' or '--' with 0.00 or any other default value
                
                if actual != 'N/A' and actual != '--':
                    actual_eps.append(float(actual.replace('$', '')))
                else:
                    actual_eps.append(0.00)  # Replace 'N/A' or '--' with 0.00 or any other default value

            # Calculate percentage movement label
                percentage_movement = []
                percentage_movement_label = []

                for low, high in zip(earnings_low, earnings_high):
                    if low == 0:  # Skip calculation if low value is zero
                        percentage_movement.append(0)
                        percentage_movement_label.append('')
                    else:
                        movement = (high - low) / low * 100
                        percentage_movement.append(movement)
                        percentage_movement_label.append(f'{round(movement)}%')
                ...
                for date, low, high in zip(dates, earnings_low, earnings_high):
                    ax1.plot([date, date], [low, high], color='lightblue', linewidth=30)
            #x = np.arange(len(dates))
            #width = 10  # Width of the bars
            #ax2.bar(x - width/2, estimated_eps, width, color='lightblue', label='Estimated EPS')
            #ax2.bar(x + width/2, actual_eps, width, color='lightgreen', label='Actual EPS')
    

            # Plot the price range as bars on the left side
            #ax1.bar(dates, earnings_high, bottom=earnings_low, width=50, color='lightblue', label='Price Range')

            # Plot bars for estimated EPS
            #ax2.bar(dates, estimated_eps, width=40, color='blue', label='Estimated EPS')
            # Plot bars for actual EPS
            #ax2.bar(dates, actual_eps, width=20, color='green', label='Actual EPS')
            ...
            # Adjust y-axis limits to include all prices
            ax1.set_xlim(min(dates), max(dates))
            ax1.set_ylim(min(earnings_low) * 0.90, max(earnings_high) * 1.10)
            #ax2.set_ylim(0, max(max(estimated_eps), max(actual_eps)) * 1.05)
            ...
            # Display the earnings chart in the GUI
            self.window['-EARNINGS-'].update(f"{''.join([f'Date: {date}, Low: {low}, High: {high}, Estimated EPS: {estimated_eps}, Actual EPS: {actual_eps}\n' for date, low, high, estimated_eps, actual_eps in earnings_data])}")
    ...
    def display_finviz_info(self, finviz_info):
        keys_to_display = ['Market Cap', 'Short Float', 'Shs Float', 'Country', 'Insider Own', 'Inst Own', 'News']
        max_key_length = max(len(key) for key in keys_to_display)
